...
internal static class BitmapTranscoder
{
    ...
    private static unsafe void TranscodeFromYUVInternal(
       ...
       TranscodeFormats conversionStandard,
        ...
        byte* pFrom, byte* pTo)
    {
        switch (transcodeFormat)
        {
            ...
            case TranscodeFormats.BT_709:
            case TranscodeFormats.BT_2020:
                TranscodeFromYUVInternal(width, height, transcodeFormat, performFullRange, isUYVY, pFrom, pTo);
                break;
            case TranscodeFormats.Auto:
                ...
                if (width > 1920 || height > 1080)  // UHD or larger
                    TranscodeFromYUVInternal(width, height, TranscodeFormats.BT_2020, performFullRange, isUYVY, pFrom, pTo);
                else if (width > 720 || height > 576) // HD
                    TranscodeFromYUVInternal(width, height, TranscodeFormats.BT_709, performFullRange, isUYVY, pFrom, pTo);
                else // SD
                    TranscodeFromYUVInternal(width, height, TranscodeFormats.BT_601, performFullRange, isUYVY, pFrom, pTo);
                break;
            default:
        ...
        byte* pFrom, byte* pTo)
    {
        switch (compression)
        {
            case NativeMethods.Compression.UYVY:
            case NativeMethods.Compression.HDYC:
                TranscodeFromYUV(width, height, transcodeFormat, true, performFullRange, pFrom, pTo);
                break;
            case NativeMethods.Compression.YUYV:
            case NativeMethods.Compression.YUY2:
                TranscodeFromYUV(width, height, transcodeFormat, false, performFullRange, pFrom, pTo);
                break;
            default:
